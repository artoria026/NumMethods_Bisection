...
function load_html(_xr,xr_anterior,_xa,_xb,_ep,_fxr,_fxa){
    ...
    <h5 class="card-title" style="color: #0d6efd;">Iteración ${index}</h5>
    <br>
    <p class="card-text">xr = (xa + xb)/2</p>
    <p class="card-text">${_xr} = (${xa} + ${xb})/2</p>
    <br>
    <p class="card-text">Ep = |(Xr(actual)+Xr(anterior))  /  (Xr(actual) * 100| </p>
    <p class="card-text">${_ep} = |(${_xr}+${xr_anterior}))  /  (${_xr}) * 100| </p>
    <br>
    <p class="card-text">f(Xa)*f(Xr)</p>
    <p class="card-text">f(${_fxa})*f(${_fxr}) = <strong>${(_fxr*_fxa)}</strong></p>
    </div>
    `
...
function load_intersection(_xa,_xb){
    ...
    <br>
    <div class="container">
    <h2 class="card-title" style="color: #0d6efd;">Intersección</h2>
    <h3 class="card-title">Cambio de signo</h3>
    <p class="card-text">xa = <strong>${_xa}</strong></p>
    <p class="card-text">xb = <strong>${_xb}</strong></p>
    </div>
    `
